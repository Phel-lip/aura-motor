...
export function DeluxeSections() {
  ...
  return <>
    ...
    <section className="section testimonials-section" id="depoimentos"><div className="section-heading"><div><p className="eyebrow">Palavras que ficam</p><h2>O que falam<br />sobre o Aura.</h2></div><p>Na versão personalizada, este espaço recebe avaliações reais dos clientes do salão.</p></div><div className="reviews-grid">{reviews.map(([, text], index) => <article className="review" key={index}><div className="google-line"><span className="review-avatar" aria-hidden="true">A</span><span>Exemplo demonstrativo</span><span className="stars">★★★★★</span></div><p>“{text}”</p><strong>Avaliação ilustrativa</strong></article>)}</div></section>
